...
public abstract class NettyRemotingAbstract {
  ...
  protected void doBeforeRpcHooks(String addr, RemoteCmd request) {
    if (rpcHooks.size() > 0) {
  ...
  protected void doAfterRpcHooks(String addr, RemoteCmd request, RemoteCmd response) {
    if (rpcHooks.size() > 0) {
      for (RPCHook rpcHook : rpcHooks) {
   ...
   */
  public void processRequestCommand(final ChannelHandlerContext ctx, final RemoteCmd cmd) {
    ...
    if (pair != null) {
      Runnable run =
          new Runnable() {
            ...
            public void run() {
              try {
                doBeforeRpcHooks(RemoteHelper.parseChannelRemoteAddr(ctx.channel()), cmd);
                final RemotingResponseCallback callback =
                    new RemotingResponseCallback() {
                      @Override
                      public void callback(RemoteCmd response) {
                        doAfterRpcHooks(
   ...
   */
  public void processResponseCommand(ChannelHandlerContext ctx, RemoteCmd cmd) {
    final int opaque = cmd.getOpaque();
  ...
  public RemoteCmd invokeSyncImpl(
    ...
    try {
      ...
      // Block
      RemoteCmd responseCommand = responseFuture.waitResponse(timeoutMillis);

      if (null == responseCommand) {
        if (responseFuture.isSendRequestOK()) {
          throw new RemoteTimeoutException(
              RemoteHelper.parseSocketAddressAddr(addr),
              timeoutMillis,
              responseFuture.getCause());
        } else {
          throw new RemoteSendRequestException(
              RemoteHelper.parseSocketAddressAddr(addr), responseFuture.getCause());
        }
      }
